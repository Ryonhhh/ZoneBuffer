...
FRAME_ID BufferManager::fix_page(bool is_write, PAGE_ID page_id) {
    ...
    if (lru) {
        ...
        if (bcb == nullptr) {
            ...
            set_page_id(frame_id, page_id);
            if (zalp_wc) (buffer + frame_id)->cluster_flag = 0;
            if (!is_write) {
                if (zalp_wc || zalp) {
            ...
            frame_id = bcb->get_frame_id();
            if (zalp_wc) (buffer + frame_id)->cluster_flag++;
...
void BufferManager::evict_victim() {
    ...
    if (zalp_wc) {
        ...
        for (auto &iter : *candidate_list) {
            if ((buffer + iter)->cluster_flag == cf)
                victim_list->push_back(iter);
        }
    } else if (cflru) {
        cf = (int)random() % CLUSTER_NUM;
        for (auto &iter : *candidate_list) {
            victim_list->push_back(iter);
        }
    }
...
void BufferManager::clean_buffer() {
    // strategy->print_list();
    std::ofstream op(zdsm->output, std::ios::app);
